...
const API_KEY = 'c7551588-e568-4868-b74c-712e5389af55&fbclid';
...
class Fetch {
  ...
  static Future updateTask(String id, String title, bool done) async {
    var response = await http.put('$API_URL/todos/$id?key=$API_KEY',
  ...
  static Future deleteTask(String id) async {
    var response = await http.delete('$API_URL/todos/$id?key=$API_KEY');
    if (response.statusCode != 200) {
      throw Exception(response.statusCode.toString());
    }
  }
